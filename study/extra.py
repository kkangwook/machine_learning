...
3. 확률적 경사하강법: 클래스가 여러개더라도 하나의 선형방정식을 만듬 
...
확률적 경사 하강법(SGD)은 단일 데이터 포인트를 사용하여 기울기를 계산
  :데이터 하나 뽑아 가중치를 업데이트하고 또 다시 다른 데이터 뽑아 가중치를 업데이트
    ...
    ->랜덤하게 뽑히면서 클래스정답이 다른 샘플들이 뽑히지만 그런 부분도 다 업데이트하여 !!하나의!! 선형방정식을 찾음
    ->시간이 지날수록 평균에 근접해지며, 이러한 하나의 전체과정epoch를 여러번 반복해서 최고의 
    전역 최솟값과 그에따른 선형방정식을 구함
그렇다면 선형방정식이 하나인데 클래스는 무엇을 기준으로 구분하나?
  ->모든 클래스에 하나의 선형방정식을 적용시켜 각 클래스별 z예측값을 구하고 소프트맥스 함수를 거쳐 
    가장 확률높은 클래스로 분류
