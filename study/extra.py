...
2. 배치 경사 하강법(Batch Gradient Descent)은 전체 데이터를 한 번에 사용하여 기울기를 계산하지만, 
**확률적 경사 하강법(SGD)**은 단일 데이터 포인트를 사용하여 기울기를 계산합니다.
이 과정에서 손실 함수의 기울기가 **잡음(noise)**을 포함하기 때문에, 지역 최소값을 탈출할 수 있는 기회가 생깁니다.
즉, SGD는 각 데이터 포인트에 대해 가중치를 업데이트하기 때문에, 일반적인 경사 하강법보다 지역 최소값에 덜 민감하고,
때로는 전역 최소값에 도달할 확률이 더 높습니다.
